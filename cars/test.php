...
switch($aa){
    case '0':
    $sql = "select * from cars where car_id like '%{$keywords}%'";
    break;
    case '1':
    $sql = "select * from cars where car_num like '%{$keywords}%'";
    break;
    case '2':
    $sql = "select * from cars where car_brand like '%{$keywords}%'";
    break;
    case '3':
    $sql = "select * from cars where color like '%{$keywords}%'";
    break;
    case '4':
    $sql = "select * from cars where sale like '%{$keywords}%'";
    break;
}
                        ...
                        <?php foreach ($rows as $row) { ?>
                            <tbody>
                                <tr>
                                    ...
                                    <td><a class="btn btn-warning  btn-xs btn-rounded" type='button' href='cars_alter.php?id=<?php echo ($row['car_id']) ?>'>修改</a> <a class="btn btn-danger  btn-xs btn-rounded" type='button' href="javascript:if(confirm('确认删除这条汽车信息么？'))location='cars_management.php?a=delete&id=<?php echo ($row['car_id']) ?>'">删除</a>
